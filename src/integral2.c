					...
					int ua, int ub)
{
	...
	e[5]=x=x2=b;
	e[6]=f2=(*fx)(x,y);
	e[4] += integralqad2(y,0,fx,e,&x0,&x1,&x2,&f0,&f1,&f2,re,ae,b1);
	if (!ub) {
		if (a < b) {
			b1=b-1.0;
			x0=1.0;
		...
		e[6]=f2=0.0;
		ae=e[2]*90.0;
		e[4] -= integralqad2(y,1,fx,e,&x0,&x1,&x2,&f0,&f1,&f2,re,ae,b1);
	}
	return e[4];
}
			...
			double *f2, double re, double ae, double b1)
{
	...
	if (transf) {
		...
		(*f1)=(*fx)(x,y);
	sum=0.0;
	integralint2(y,transf,fx,e,x0,x1,x2,f0,f1,f2,&sum,re,ae,b1,hmin);
	return sum/180.0;
}
			...
			double hmin)
{
	...
	while (anew) {
		...
		if (fabs(t) < fabs(v)*re+ae)
			...
			integralint2(y,transf,fx,e,x0,x1,x2,f0,f1,f2,sum,
							re,ae,b1,hmin);
			*x2=x3;
			*f2=f3;
			anew=1;
